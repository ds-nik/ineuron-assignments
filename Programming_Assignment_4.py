...
class Programming_Assignment_4:
    ...
    def q3(self):
        ...
        try:
            ...
            l = [0, 1]
            if n<=0:
                logging.info("Enter the positive integer value")
            elif n == 1:
                l1 = [0]
                logging.info("list of first %s digits of fibonacci sequence is %s", n, l1)
            else:
    ...
    def q3_alternate(self):
        #fibonacci sequence program using while loop
        try:
            n = int(input("Enter the number of digits of fibonacci sequence desired"))
            i = 1
            a, b = 0, 1
            l = [0,1]
            if n <= 0:
                logging.info("Enter the positive integer value of 'n'")
            elif n == 1:
                logging.info("First number of fibonacci sequence is %s", a)
            elif n == 2:
                logging.info("First two digits of fibonacci sequence is %s , %s",a,b)
            else:
                while i <= n-2:
                    s = l[i] + l[i-1]
                    i = i+1
...
#Programming_Assignment_4().q3_alternate()
